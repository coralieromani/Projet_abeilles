...
\newpage
# Introduction
L'étude a pour objectif d'analyser les changements de poids dans une ruche durant une journée selon différents facteurs environnementaux afin de mesurer leurs effets sur les colonies d'abeilles.\newline
...
# Modélisation 
L'objectif est de modéliser le changement de poids par régression par morceaux. Pour obtenir un ensemble de données indépendantes, ils commençent par éliminer la tendance en soustrayant le poids enregistré à minuit de celui enregistré à chaque instant de la journée.\newline
...
## Fonction "segmented"
Le changement de poids dans la journée peut se modéliser dans R par la fonction "segmented". Cette fonction peut aussi déterminer le nombre de points de rupture mais elle tend à surestimer le nombre pour que l'estimation soit préférable pour le modèle final.\newline
...
JE SAIS PAS SI FAUT FAIRE CETTE PARTIE

La pente de la régression est associée aux mouvements net des abeilles. En effet, la valeur absolue de la pente représente la plus grande masse par habitant des abeilles rentrant avec du pollen ou du nectar comparés à ceux qui partent.
...
JE SUIS PERDUE POUR CETTE FIGURE, À L'AIDE