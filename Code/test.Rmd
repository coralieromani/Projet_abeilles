...
x <- 1:100
y <- 2 + 1.5 * pmax(x - 35, 0) - 1.5 * pmax(x - 70, 0) + rnorm(100, 0, 2)

# Ajustement d'un modèle linéaire initial
mod_lm <- lm(y ~ x)

# Ajustement du modèle segmenté avec deux points de rupture initiaux à 30 et 60
library(segmented)
mod_seg <- segmented(mod_lm, seg.Z = ~x, psi = c(30, 60))
...
# Estimation automatique des points de rupture (ici, le package choisira le nombre de ruptures)
mod_seg_auto <- segmented(mod_lm, seg.Z = ~x)
...
On peut choisir le nombre de beaking points nous oushaitons et il nous les évalue.
