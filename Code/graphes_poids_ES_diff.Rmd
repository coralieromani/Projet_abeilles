...
graph_entrees_sorties <- ggplot(sica_moyenne, aes(x = heure, y = moyenne_valeur, color = mouvement)) +
  ...
  labs(x = NULL, y = "Activité", color = "Mouvement") +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank())
...
graph_diff <- ggplot(sica_diff, aes(x = heure, y = diff, color = "Différence")) +
  geom_point(size = 2) +
  scale_color_manual(values = c("Différence" = "orange")) +
  labs(x = "Heure", y = "Entrées - Sorties", color = "") +
  theme_minimal()

# Affichage combiné
(graph_poids / graph_entrees_sorties / graph_diff) + plot_layout(guides = "collect") & theme(legend.position = "bottom")
