...
sica_diff = readRDS("sica_diff.rds")
```

# Différences des entrées/sorties entre le BP1 et le BP2 en fonction de la pente du BC entre ces 2 BP
```{r}
breakpoints_table = breakpoints_table %>% mutate(diff_BP2_1 = diff_ES_BP_1 - diff_ES_BP_2)

ggplot(breakpoints_table, aes(x = variation_poids, y = diff_BP2_1)) + geom_point(colour ="skyblue") + theme_minimal()
```
...
```{r}
ggplot(breakpoints_table, aes(x = variation_poids, y = diff_BP2_1)) + geom_point(colour ="skyblue") +
  scale_x_continuous(limits = c(-0.75, 0.05)) + theme_minimal()
```
...
```{r}
ggplot(breakpoints_table, aes(x = variation_poids, y = pente_ES)) + geom_point(colour ="skyblue") +
  scale_x_continuous(limits = c(-0.75, 0.05)) + theme_minimal()
```
...
ggplot(breakpoints_table_entree, aes(x = variation_poids, y = entree_BP2_1)) + geom_point(colour ="skyblue") + theme_minimal()
```
...
# Différences des pentes entrées/sorties entre le BP1 et le BP2 en fonction de la pente du BC entre ces 2 BP (sans valeurs extrêmes)
```{r}
ggplot(breakpoints_table_entree, aes(x = variation_poids, y = pente_entree)) + geom_point(colour ="skyblue") +
  scale_x_continuous(limits = c(-0.75, 0.05)) + theme_minimal()
```
...
ggplot(breakpoints_table_sortie, aes(x = variation_poids, y = sortie_BP2_1)) + geom_point(colour ="skyblue") + theme_minimal()
```
...
# Différences des pentes entrées/sorties entre le BP1 et le BP2 en fonction de la pente du BC entre ces 2 BP (sans valeurs extrêmes)
```{r}
ggplot(breakpoints_table_sortie, aes(x = variation_poids, y = pente_sortie)) + geom_point(colour ="skyblue") +
  scale_x_continuous(limits = c(-0.75, 0.05)) + theme_minimal()
```