...
# Création du graphique avec des courbes
graphique_global <- ggplot(sica_moyenne, aes(x = date, y = moyenne_valeur, color = Mouvement)) +
  geom_line(size = 0.5) +  # Courbes des moyennes
  scale_color_manual(values = c("Entrées" = "purple", "Sorties" = "pink")) +  # Couleurs
  scale_x_datetime(date_labels = "%H:%M") +  # Afficher uniquement les heures sur l'axe X
...
# Calculer la moyenne des entrées et sorties à chaque instant
sica_moyenne2 <- sica_10avril %>%
  group_by(date, Mouvement) %>%
  summarise(moyenne_valeur = mean(valeur, na.rm = TRUE), .groups = "drop")

# Création du graphique avec des courbes
graphique_global2 <- ggplot(sica_moyenne2, aes(x = date, y = moyenne_valeur, color = Mouvement)) +
  geom_line(size = 0.5) +  # Courbes des moyennes
  scale_color_manual(values = c("Entrées" = "purple", "Sorties" = "pink")) +  # Couleurs
  scale_x_datetime(date_labels = "%H:%M") +  # Afficher uniquement les heures sur l'axe X
