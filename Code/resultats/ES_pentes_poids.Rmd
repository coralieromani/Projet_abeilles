...
```{r}
ggplot(breakpoints_table, aes(x = variation_poids, y = somme_diff_ES, colour = factor(mois))) + geom_point() + labs(y = "Somme cumulée des différences Entrées/Sorties", x = "Pente du Breakfast Canyon", colour = "Mois") + theme_minimal()
```
...
ggplot(breakpoints_table, aes(x = variation_poids, y = somme_diff_ES, colour = factor(mois))) + geom_point()  + scale_x_continuous(limits = c(-0.35, 0.05)) + geom_smooth(method="lm", se =FALSE) +
  labs(y = "Somme cumulée des différences Entrées/Sorties", x = "Pente du Breakfast Canyon", colour = "Mois") + theme_minimal()
```
